...
#define DEF_DTH_BBY 0.1
#define DEF_DTH_BLOB 0.25
#define DEF_DTH_OLD 0.4
...
int main(int argc, char *argv[]) {
	...
	Food fud;
*/
	userData uData = { DEF_MODE, DEF_INIT_BLOB, DEF_FOODCNT, DEF_SPD_MAX, DEF_SPD_PROP, 
											...
											while (running)
											{
												...
												while (al_get_next_event(queue, &ev))
												{
													...
													if ((ev.type == ALLEGRO_EVENT_TIMER)&&( blobWorld.food != NULL )) {	
														...
														blobWorld.moveBlobs(uData.smellRadius);
														//blobWorld.feedBlobs(uData.deathProbBabyBlob, uData.mode, uData.speedMax, uData.speedProb);
														//blobWorld.manageFood(uData.foodCount);
														//blobWorld.blobCollision(uData.randomJiggleLimit, uData.deathProbGrownBlob, uData.deathProbOldBlob);
														blobWorld.killBlobs();
														refresh = true;
													}
												...
												if (state) {
													...
													if(!uData.mode)
														...
														ImGui::SliderInt(" ", &uData.speedMax, 1, 1, NULL, reg_slider);
